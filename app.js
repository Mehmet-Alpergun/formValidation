...
let verifyPasswordInput = document.getElementById("password");
...
let invalidVClasses = document.getElementsByClassName("error");

const passwordVerify = (password) => {
  const regex =
    /^(?=.+[a-z])(?=.+[A-Z])(?=.+[0-9])(?=.+[\$\%\^\&\!@\#\*\(\)\+\=`~\?\>\<])/;
  return regex.test(password) && password.lentgh >= 8;
};
...
verifyPasswordInput.addEventListener("input", () => {
  if (verifyPasswordInput.value === passwordInput.value) {
    verifyPasswordError.classList("hide");
...
submitButton.addEventListener("click", () => {
  if (validClasses.lentgh == 6 && invalidVClasses.length == 0) {
    alert("Success");
  } else {
    alert("Error");
  }
